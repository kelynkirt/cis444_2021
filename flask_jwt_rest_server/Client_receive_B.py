...
Algorithm for Client_receive_B program
...
current_node_accts = ["B0000001", "B0000002"]  # used to reference the accounts for this node
other_node_accts = ["A0000001", "A0000002"]  # used to reference the accounts for the 2nd node
unconfirmed_acct_balances = {
    'B0000001': '000003E8',  # this amounts to 1000 BC
    'B0000002': '000003E8'  # this amounts to 1000 BC
}
...
def get_acct_balances():
    with open('balances_B.txt', 'r') as f:  # opens the txt file storing the balances in read only mode
...
def set_balances():
    with open('balances_B.txt', 'w') as f:  # open the balances file, should completely rewrite the txt file
        # will be in format:
...
def add_to_balances(amount, payee):
    with open('balances_B.txt', 'w') as f:  # open the balances file, should completely rewrite the txt file
        ...
        if current_node_accts[0] == payee:  # if it is account 1
            ...
            unconfirmed_bal = int(unconfirmed_acct_balances[current_node_accts[0]], 16)  # converts the hex string to an int
        ...
        if current_node_accts[1] == payee:  # if it is account 2
            # add amount to both unconfirmed and confirmed balances
            unconfirmed_bal = int(unconfirmed_acct_balances[current_node_accts[1]], 16)  # converts the hex string to an int, no need for the leading 0x for this to work
...
def get_unconfirmed_txs():
    with open('Unconfirmed_T_B.txt', 'r') as f:  # open the unconfirmed transaction txt file
        # f.readlines() creates a list of all the lines in the file (including \n)
...
def set_unconfirmed_txt(unconfirmed_txs):
    with open('Unconfirmed_T_B.txt', 'w') as f:  # open the unconfirmed transaction file
...
def append_to_confirmed_tx_file(transaction):
    with open('Confirmed_T_B.txt', 'a') as f:  # opens the file containing the confirmed transactions
...
while True:
    ...
    transaction = transaction.decode()  # !!!! MAY NEED TO PUT THIS INTO A WHILE LOOP UNTIL 'EOF' IS ENCOUNTERED !!!
