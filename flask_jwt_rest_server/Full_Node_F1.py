...
e.g. 10000 for F1 and 1100 for F2.  Recall that the full nodes take turns to mine blocks.
...
General Algorithm:
--------------------------------------------------------------------------------------------------------
In an infinite loop, the program listens to the requests for connections, which could be from its client6
...
mining_fee = 30
total_tx_fee = 8
client_accts = ['A0000001', 'A0000002']


def TTT(transaction):
    # receive the tx and append it to the Temp_T.txt
    with open('Temp_T_F1.txt', 'a') as f:  # opens the file containing the unconfirmed transactions
    ...
    if num_temp_tx % 4 == 0:
        ...
        turn += 1
        if turn % 2 == 1:  # it is this node's turn to mine
            ...
            with open('balance_F1.txt', 'w') as f:  # opens the file containing the balance
...
def append_to_blockchain(block):
    with open('Blockchain_F1.txt', 'a') as f:  # opens the file containing the block chain
        f.write(block + "\n")  # appends/writes the block to the file


def remove_temp_txs():
    with open('Temp_T_F1.txt', 'r') as f_old:  # opens the balances txt file
    ...
    with open('Temp_T_F1.txt', 'w'):  # just doing this should clear the file must be after the read functions
...
def generate_merkle_root(tx_list):
    ...
    hash_cd = hash_handler.hexdigest()  # has that string and store
...
def get_last_block_hash():
    hash_handler = hashlib.sha256()
    with open('Blockchain_F1.txt', 'r') as f:
...
def get_client_txs_from_list(txs_from_block):
    client_txs_list = []
    # recall from above client_accts = ['A0000001', 'A0000002']
