...
Algorithm for Client_send_B.py

The client will have three files:
1. Unconfirmed_T_B.txt to store the unconfirmed transactions
2. Confirmed_T_B.txt to store the confirmed transactions
3. balances_B.txt
...
For example, for Client B, the file will be initialized as following. We store the values in HEX.
B0000001:000003E8:000003E8
...
current_node_accts = ["B0000001", "B0000002"]
other_node_accts = ["A0000001", "A0000002"]
...
serverPort = 11000  # hw instructions suggest port 11000
...
def send_tx_to_node(tx):
    ...
    clientSocket.sendto(tx.encode(), (serverName, serverPort))  # encodes and sends the transaction information to
    # the full node
...
def get_acct_balances():
    with open('balances_B.txt', 'r') as f:  # opens the balances txt file
...
def get_unconfirmed_txs():
    with open('Unconfirmed_T_B.txt', 'r') as f:  # opens the unconfirmed transactions text file
        # f.readlines() creates a list of all the lines in the file (including \n)
...
def get_confirmed_txs():  # opens the confirmed transactions text file
    with open('Confirmed_T_B.txt', 'r') as f:
        # f.readlines() creates a list of all the lines in the file (including \n)
...
while True:
    ...
    if choice == 1:  # user wishes to enter a new transaction
        ...
        if tx_total <= unconfirmed_bal:  # accessing the value in the dictionary using the key
            ...
            unconfirmed_acct_balances[
                tx_payer_acct] = unconfirmed_bal.zfill(8)  # stores that amount back into the unconfirmed balance dictionary
            ...
            tx = tx_payer_acct + tx_payee_acct + tx_amount.zfill(8)  # creates the 12 byte string that represents the transaction
            with open('Unconfirmed_T_B.txt', 'a') as f:  # opens the unconfirmed transaction file
